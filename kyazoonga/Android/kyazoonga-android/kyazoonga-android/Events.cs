...
//			SetContentView (textview);
			...
			for (int i=0; i<categoriesList.Capacity; i++) {
				items.Add (new Tuple<string, string, string,int,string> (((CategoryList)categoriesList [i]).title, "venus :" + ((CategoryList)categoriesList [i]).venue, "date :26-11-2013", Resource.Drawable.loifl,((CategoryList)categoriesList [i]).id));
